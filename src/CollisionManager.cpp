		...
		)
	{
		if (!object2->getIsColliding()) {
			...
			case PLATFORM:
			{
				...
				TheSoundManager::Instance()->playSound("yay", 0);
				Hero* pHero = dynamic_cast<Hero*>(object1);
				if (pHero != nullptr)
				{
					pHero->SetOnGround(true);
					pHero->SetJump(false);
					pHero->setVelocity(glm::vec2(pHero->getVelocity().x, 0.f));
					pHero->setPosition(glm::vec2(pHero->getPosition().x,
						object2->getPosition().y - object2->getHeight() * 0.5f - pHero->getHeight() * 0.5f));
				}
			}
				break;
			default:
